...
import com.azheng.boot.rag.enums.IntentLevel;
import com.azheng.boot.rag.core.intent.IntentNode;
import com.azheng.boot.rag.core.intent.IntentTreeCacheManager;
import com.azheng.boot.rag.core.intent.IntentTreeFactory;
...
import static com.azheng.boot.rag.enums.IntentLevel.DOMAIN;
...
public class IntentTreeServiceImpl extends ServiceImpl<IntentNodeMapper, IntentNodeDO> implements IntentTreeService {
     ...
     * 获取意图树
     * 1.升序获取所有节点
     * 2.按父节点进行分组，默认父节点为ROOT，获取 父节点标识-对应节点集合
     * 3.返回ROOT键值的映射，没有映射返回空
     * @return
     */
    @Override
    ...
    public String createNode(IntentNodeCreateRequest requestParam) {
        ...
        long count = this.count(new LambdaQueryWrapper<IntentNodeDO>()
                ...
                .eq(IntentNodeDO::getDelFlag, 0));
        if (count > 0) {
            throw new RuntimeException("意图标识已存在: " + requestParam.getIntentCode());
        }
        ...
        if (Objects.equals(requestParam.getLevel(), DOMAIN.getCode())
            ...
            throw new RuntimeException("Domain类型的RAG检索意图识别时，必须指定目标知识库");
        }
        Long kbId = StrUtil.isNotBlank(requestParam.getKbId()) ? Long.parseLong(requestParam.getKbId()) : null;
        ...
        if (kbId != null) {
            ...
            node.setCollectionName(knowledgeBasePO.getCollection());
        }
        this.save(node);
    ...
    public void updateNode(String id, IntentNodeUpdateRequest req) {
        ...
        if (node == null || Objects.equals(node.getDelFlag(), 1)) {
            throw new RuntimeException("节点不存在或已删除: id=" + id);
        }
    ...
    private List<IntentNodeDO> listAndValidateTargetNodes(List<Long> ids) {
        ...
        if (targetNodes.size() != normalizedIds.size()) {
            ...
            List<Long> missingIds = normalizedIds.stream()
                    ...
                    .limit(5)
                    .toList();
            throw new RuntimeException("节点不存在或已删除: " + missingIds);
        }
        return targetNodes;
    }
