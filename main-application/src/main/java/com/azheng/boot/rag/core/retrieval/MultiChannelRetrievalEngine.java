...
import com.azheng.boot.rag.core.retrieval.postprocessor.SearchResultPostProcessor;
...
public class MultiChannelRetrievalEngine {
     ...
     * 执行多通道检索（仅 KB 场景）
     *
     * @param subIntents 子问题意图列表 传到这的都是单个子问题，只不过封装成了List，实际只有一个
     * @param topK       期望返回的结果数量
     * @return 检索到的 Chunk 列表
     */
    public List<ReRankContext> retrieveKnowledgeChannels(List<SubQuestionIntent> subIntents, int topK) {
        // 构建检索上下文
     ...
     * 执行后置处理器链
     */
    private List<ReRankContext> executePostProcessors(List<SearchChannelResult> results,
        ...
        List<ReRankContext> chunks = results.stream()
                .flatMap(r -> r.getChunks().stream())
                .collect(Collectors.toList());
